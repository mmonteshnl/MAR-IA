      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6 gap-x-5 gap-y-5 px-1 pb-4">
        {LEAD_STAGES.map(stage => (
          <div key={stage} className="flex-shrink-0 w-full inline-block align-top"> {/* Ensure columns are laid out horizontally for scroll */}
            <Card className="bg-card border-border/30 shadow-none min-h-[300px] flex flex-col rounded-[var(--radius)] h-full"> {/* Added h-full */}
              <CardHeader className="pb-3 pt-4 px-4 sticky top-0 bg-card z-10 border-b border-border/20 rounded-t-[var(--radius)]">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center">
                    <span className={`mr-2.5 h-2.5 w-2.5 rounded-full ${stageColors[stage]?.dotColor || 'bg-muted-foreground'}`}></span>
              ...
              <CardContent className="space-y-3 flex-grow overflow-y-auto p-3 pt-3">
                  ...
                  return (
                    ...
                    >
                      <CardHeader className="p-3 flex flex-row items-start justify-between space-x-2">
                        <div className="flex-grow space-y-0.5 min-w-0">
                            <h3 className="font-semibold text-sm text-foreground leading-tight flex items-center truncate" title={lead.name}>
                              ...
                              {isImportedIncomplete && <AlertCircle className="h-3.5 w-3.5 text-orange-400 ml-1 flex-shrink-0" title="Datos incompletos" />}
                            </h3>
                      ...
                      <CardContent className="p-3 pt-1">
                        <div className="flex items-center justify-start space-x-1 text-xs text-muted-foreground mb-2.5">
                            ...
                            {generateWhatsAppLink(lead.phone, lead.name) && ( 
                                <a href={generateWhatsAppLink(lead.phone, lead.name)!} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} title="Enviar WhatsApp" className="p-1 hover:text-primary rounded-full hover:bg-primary/10">
                            ...
                            {(isFieldMissing(lead.phone) && isFieldMissing(lead.website) && isFieldMissing(lead.email) && !generateWhatsAppLink(lead.phone, lead.name)) && <span className="text-xs text-muted-foreground/70 italic pl-1">Sin contacto directo</span>}
                          </div>
                          <LeadActionButtons
                              lead={lead}
                      ...
                      <CardFooter className="p-3 border-t border-border/20 mt-auto"> {/* Added mt-auto to push footer down */}
                        <Select value={lead.stage} onValueChange={(newStage) => onStageChange(lead.id, newStage as LeadStage)} onClick={e => e.stopPropagation()}>
